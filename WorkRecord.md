...
* 小程序端
  * 组织相关
    * 优先级中：上传、修改*组织头像*
    * 优先级低：组织发通知公告？
  * 优先级中：更新UI设计（act-list、my-home）、时间格式化（例如改为”几小时前“、”昨天“）、组织名称最大字数（关注页面、推荐页面）、日历有活动的日期加标志、用户个人页面
  * 优先级高：**用户身份（合并授权和邮箱验证，不验证你就啥也别干）**
...
* 后端
  ...
  * 优先级中：合并原子操作
    ...
    * 防止bug，如：先后发“创建组织”“增加创始人为管理员”两个请求，若中间中断则组织无管理员
  * 优先级高：**安全相关**
    * 例如获取所有用户信息等各种api需要验证token
