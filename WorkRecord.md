...
| 开聚实 | web；周报、汇报ppt                   |      |
| 王肇凯 | 小程序                               |      |
| 王佳瑞 | 后端（图片上传、导出每个页面二维码） |      |
| 李永毅 | 后端（操作日志功能、token验证）      |      |
...
* 小程序端
  * 组织相关
    * 优先级中：上传、修改*组织头像*
    * 优先级低：组织发通知公告？
  * 优先级中：更新UI设计（act-list、my-home）、时间格式化（例如改为”几小时前“、”昨天“）、组织名称最大字数（关注页面、推荐页面，省略号）、用户个人页面、活动页面返回对应组织页面
  * 优先级高：**用户身份（合并授权和邮箱验证，不验证只有查看权限）**
  * 优先级高：**小程序内通知功能、小程序外提醒**
  * 优先级高：**活动地图**
  * 活动&组织
    * 优先级高：**活动/组织分享**、导出为二维码、扫码跳转至活动/组织页面
    * 优先级高：**搜索活动/组织功能**
