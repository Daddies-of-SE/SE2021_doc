...
| **NO** | **类型** | **信息内容** | **信息表现** | **说明**                                       |
| ------ | -------- | ------------ | ------------ | ---------------------------------------------- |
| 1      | 必选     | 评论删除     | 删除按键     | 用户自己的评论旁有删除按键，点击后即可删除评论 |
...
点击进入“我的”界面，进入“我管理的活动”界面，在“未进行”的活动中查找需要删除的活动，点击“删除”按键。在弹出的对话框中点击“确认”按键。

#### 3.19.3 用户界面设计

| **NO** | **类型** | **信息内容** | **信息表现** | **说明**                                         |
| ------ | -------- | ------------ | ------------ | ------------------------------------------------ |
| 1      | 必选     | 活动删除     | 删除按键     | 用户所管理的活动旁有删除按键，点击后即可删除活动 |
...
| **NO** | **类型** | **信息内容**  | **信息表现** | **说明**                                                     |
| ------ | -------- | ------------- | ------------ | ------------------------------------------------------------ |
| 1      | 必选     | 活动/组织分享 | 分享按键     | 每个活动、组织的标题旁有删除按键，点击后即可删除相应的活动、组织 |
...
**注**：本节仅列出接口名称及其简单说明，其详细定义和说明见“**后端接口说明**”文件。
...
| 14   | 内部 | 获取所有组织申请           | 后端接口说明  | 用于获取所有的组织申请                                       |
| 15   | 内部 | 删除指定组织申请           | 后端接口说明  | 删除编号为id的组织申请                                       |
| 16   | 内部 | 审批组织申请               | 后端接口说明  | 审核指定id的组织申请                                         |
| 17   | 内部 | 新建组织                   | 后端接口说明  | 组织申请审批通过后创建组织                                   |
| 18   | 内部 | 获取指定组织信息           | 后端接口说明  | 获取编号为id的组织的信息                                     |
| 19   | 内部 | 获取指定版块下所有组织     | 后端接口说明  | 获取对应id的版块下的所有组织                                 |
| 20   | 内部 | 修改指定组织信息           | 后端接口说明  | 修改对应id的组织信息                                         |
| 21   | 内部 | 删除指定组织               | 后端接口说明  | 删除编号为id的组织                                           |
| 22   | 内部 | 组织负责人转让             | 后端接口说明  | 将编号为id的组织的负责人更改为指定用户                       |
| 23   | 内部 | 用户关注组织               | 后端接口说明  | 用户关注组织                                                 |
| 24   | 内部 | 用户取消关注组织           | 后端接口说明  | 对应id用户取消关注对应id组织                                 |
| 25   | 内部 | 获取指定用户关注的所有组织 | 后端接口说明  | 获取编号为id的用户的关注的所有组织                           |
| 26   | 内部 | 组织添加管理员             | 后端接口说明  | 指定组织添加指定用户为管理员                                 |
| 27   | 内部 | 取消组织管理员             | 后端接口说明  | 取消对应id用户的对应id组织的管理员身份                       |
| 28   | 内部 | 获取指定用户管理的所有组织 | 后端接口说明  | 获取编号为id的用户的管理的所有组织                           |
| 29   | 内部 | 获取指定组织的全部管理员   | 后端接口说明  | 获取编号为id的组织的全部管理员                               |
| 30   | 内部 | 获取所有活动分类           | 后端接口说明  | 获取所有分类的id和名称                                       |
| 31   | 内部 | 新建活动分类               | 后端接口说明  | 创建一个新的活动分类                                         |
| 32   | 内部 | 修改指定活动分类信息       | 后端接口说明  | 修改编号为id的活动分类                                       |
| 33   | 内部 | 删除指定活动分类           | 后端接口说明  | 删除编号为id的活动分类                                       |
| 34   | 内部 | 获取所有活动地址           | 后端接口说明  | 获取所有活动地址的id、名称、经度、纬度                       |
| 35   | 内部 | 新建活动地址               | 后端接口说明  | 创建一个新的活动地址                                         |
| 36   | 内部 | 删除指定活动地址           | 后端接口说明  | 删除编号为id的活动地址                                       |
| 37   | 内部 | 新建活动                   | 后端接口说明  | 新建一个活动                                                 |
| 38   | 内部 | 获取指定活动信息           | 后端接口说明  | 获取编号为id的活动的信息                                     |
| 39   | 内部 | 修改指定活动信息           | 后端接口说明  | 修改对应id的活动信息                                         |
| 40   | 内部 | 删除指定活动               | 后端接口说明  | 删除编号为id的活动                                           |
| 41   | 内部 | 获取指定组织下所有活动     | 后端接口说明  | 获取对应id的组织下的所有活动                                 |
| 42   | 内部 | 获取指定用户发布的所有活动 | 后端接口说明  | 获取对应id的用户发布的所有活动                               |
| 43   | 内部 | 获取指定版块下的所有活动   | 后端接口说明  | 获取对应id的用户发布的所有活动                               |
| 44   | 内部 | 用户加入非审核活动         | 后端接口说明  | 用户加入一个无需审核的活动                                   |
| 45   | 内部 | 用户退出活动               | 后端接口说明  | 对应id用户退出对应id的活动                                   |
| 46   | 内部 | 获取指定用户参与的所有活动 | 后端接口说明  | 获取编号为id的用户加入的所有活动                             |
| 47   | 内部 | 获取指定活动的报名人数     | 后端接口说明  | 获取编号为id的活动的报名人数                                 |
| 48   | 内部 | 获取所有用户反馈           | 后端接口说明  | 获取所有用户反馈                                             |
| 49   | 内部 | 新建用户反馈               | 后端接口说明  | 创建一个新的用户反馈                                         |
| 50   | 内部 | 删除指定用户反馈           | 后端接口说明  | 删除编号为id的用户反馈                                       |
| 51   | 内部 | Super User注册             | 后端接口说明  | 超级管理员注册                                               |
| 52   | 内部 | SuperUser登录              | 后端接口说明  | 超级管理员登录                                               |
